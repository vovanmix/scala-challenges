...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.toList
      .groupBy((elt: Char) => elt).toList
      .sortBy(_._1).map(elt => (elt._1, elt._2.length))

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s.mkString("") |> wordOccurrences
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x :: xs => x match {
      case (char, count) =>
        for {
          rest <- combinations(xs)
          i <- 0 to count
        } yield if (i > 0) (char, i) :: rest else rest
    }
  }
...
//    }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x.toMap) {
      (acc, rec) => acc.apply(rec._1) - rec._2 match  {
        case 0 => acc - rec._1
        case diff => acc.updated(rec._1, diff)
      }
    }.toList.sorted
