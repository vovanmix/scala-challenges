...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'makeIndex' creates the correct index") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java")
    val articles = List(
    ...
    val answerList = List(
      ("Scala", List(articles.tail.head, articles.tail.tail.head)),
      ("Java", List(articles.tail.head))
    )
    val answer = sc.parallelize(answerList)

    assert(answer.collect().toSet == index.collect().toSet)
  }
