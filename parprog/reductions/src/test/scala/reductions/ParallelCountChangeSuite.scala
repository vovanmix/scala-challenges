...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("parallel using moneyThreshold") {
    def check(money: Int, coins: List[Int], expected: Int): Unit = {
      val threshold = moneyThreshold(money)
  ...
  test("parallel using totalCoinsThreshold") {
    def check(money: Int, coins: List[Int], expected: Int): Unit = {
      val threshold = totalCoinsThreshold(money)
      assert(parCountChange(money, coins, threshold) == expected,
        s"countChange($money, $coins) should be $expected")
    }
  ...
  test("parallel using combinedThreshold") {
    def check(money: Int, coins: List[Int], expected: Int): Unit = {
