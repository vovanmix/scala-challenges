...
object ParallelCountChangeRunner {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def measureParallelCountChange(threshold: ParallelCountChange.Threshold): Unit = {
      ...
      println(s"$threshold parallel result = $parResult")
      println(s"$threshold parallel count time: $fjtime ms")
      println(s"$threshold speedup: ${seqtime / fjtime}")
    }
...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if(money < 0 || coins.isEmpty || threshold(money, coins)) countChange(money, coins)
      ...
      val (l, r) = parallel[Int, Int](parCountChange(money - coins.head, coins, threshold),
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (money: Int, _: List[Int]) => money <= startingMoney * 2 / 3
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = (money, coins) => {
    val remaining = money * coins.length
    val start = startingMoney * allCoins.length
    remaining <= start / 2
  }
}
