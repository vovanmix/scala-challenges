...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel " +
    ...
    assert(boxBlurKernel(src, 2, 1, 1) === 4, "")
    assert(boxBlurKernel(src, 0, 2, 1) === 5, "")
    assert(boxBlurKernel(src, 1, 2, 1) === 5, "")
    assert(boxBlurKernel(src, 2, 2, 1) === 6, "")
  }
  ...
  test("HorizontalBoxBlur.blur with radius 1 should correctly blur the entire 3x3 image") {
    ...
    HorizontalBoxBlur.blur(src, dst, 0, h, 1)
  ...
  test("#HorizontalBoxBlur.parBlur") {
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
